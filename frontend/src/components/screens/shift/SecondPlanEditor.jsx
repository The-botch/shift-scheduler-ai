...
const SecondPlanEditor = ({ selectedShift, onNext, onPrev, mode = 'edit' }) => {
  ...
  const {
    ...
    dayShifts, setDayShifts,
    hasSavedDraft, setHasSavedDraft,
    windowState, setWindowState,
    shiftData, setShiftData,
    defaultPatternId, setDefaultPatternId,
    preferences, setPreferences,
    shiftPatterns, setShiftPatterns,
    year, month, planId, planType,
    isViewMode, isEditMode,
    preferencesMap, timeOverlapInfo,
    // マスタデータ
    staffMap, storesMap, availableStores, selectedStores,
    loadMasterData, setSelectedStores,
    // シフト編集
    addedShifts, hasUnsavedChanges, saving,
    planIds: planIdsState, modalState,
    setPlanId: setPlanIdsState,
    handleModifyShift, saveChanges, setModalState, resetChanges, setHasUnsavedChanges,
    // ベース関数（ローカルでラップする用）
    handleDeleteShiftBase, handleAddShiftBase,
    // ナビゲーション
    navigate, navigateToDashboard, handleDashboard,
    // ハンドラー（共通）- ローカルで上書きしないものはここから使用
    handleDayClick, handleMaximize, handleBack, handleDelete, handleExportCSV,
    // ヘルパー
    updateCalendarData,
  } = editor
