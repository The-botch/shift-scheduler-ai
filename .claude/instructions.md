...
### 0. データベース操作の禁止（絶対厳守）
- **絶対にデータベースに直接接続してはいけない**
- **絶対にSQLを直接実行してはいけない**
- データベース操作が必要な場合は、以下のみ許可：
  - SQLファイルの生成・作成（実行はユーザーが行う）
  - スクリプトの作成（実行はユーザーが行う）
  - 既存のスクリプトの確認・レビュー
- データベース接続情報（接続文字列、パスワード等）を含むコードは書かない
- ユーザーがデータベース操作を依頼した場合は、「SQLファイルを生成してユーザーが実行する形でよいか」確認する
...
2. **影響調査（最重要）**
   ...
   - **調査完了後、ユーザーに調査結果を報告し、確認を得る**
   - ユーザーへの確認依頼時には以下を提示：
     - 「影響調査が完了しました。以下の点をご確認ください：」
     - 「1. 影響範囲のリストに漏れがないか」
     - 「2. 想定外の影響がないか」
     - 「3. 調査結果に問題なければ、設計書作成に進みます」
...
5. **承認取得**
   ...
   - ユーザーへの確認依頼時には以下を提示：
     - 「設計書と実行計画を作成しました。以下の点をご確認ください：」
     - 「1. 設計書の内容（目的、影響範囲、リスク対策）に問題がないか」
     - 「2. 実行計画（コミット分割、順序、依存関係）は適切か」
     - 「3. 承認いただければ、実装を開始します」
...
6. **実装開始**
   ...
   - 実行計画通りの順序でコミットを作成
   - **ステップごとにユーザー確認を実施**：
     - 各ステップ（または各コミット）の作業が完了したら、必ずユーザーに報告
     - ユーザーへの確認依頼時には以下を提示：
       - 「〇〇の修正が完了しました。以下の点をご確認ください：」
       - 「1. 修正内容が設計書通りに実装されているか」
       - 「2. 意図しない変更が含まれていないか」
       - 「3. （該当する場合）動作確認の方法」
       - 「4. 問題なければ次のステップに進みます」
     - ユーザーの確認が得られてから次のステップに進む
...
### 5. デバッグコードの管理
- **デバッグ用のコード（ロガー、console.log等）を追加する場合の必須ルール**：
  - デバッグコードには必ず `// DEBUG:` または `/* DEBUG */` のコメントを付ける
  - 例：`console.log('DEBUG: user data:', userData); // DEBUG: 削除予定`
  - 例：`logger.debug('DEBUG: API response:', response); // DEBUG: 削除予定`
- **コミット前の必須確認**：
  - `git diff` でコミット対象のコードを確認
  - `// DEBUG:` または `/* DEBUG */` のコメントがないか検索
  - デバッグコードが見つかった場合は、必ず削除してからコミット
  - Grep ツールで `DEBUG:` を検索し、削除漏れがないか確認
- **絶対に本番コードにデバッグコードを残さない**
...
7. **コミット前のデバッグコード確認**
   - `git diff` でコミット対象のコードを確認
   - Grep ツールで `DEBUG:` を検索し、デバッグコードがないか確認
   - デバッグコードが見つかった場合は削除
8. コミット・プッシュ
